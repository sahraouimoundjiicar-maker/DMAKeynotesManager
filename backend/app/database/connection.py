...
Rôle :
    ...
    - Créer la BD DMAKeynotesManager et toutes ses tables (uniquement en local)
...
# ─────────────────────────────────────────────────────────────
# ÉTAPE 2 — CRÉATION DE LA BASE DE DONNÉES (LOCAL UNIQUEMENT)
# ─────────────────────────────────────────────────────────────
...
def creer_base_de_donnees() -> None:
    ...
    ⚠️ Cette fonction n'est utilisée qu'en environnement local.
...
# ─────────────────────────────────────────────────────────────
# ÉTAPE 8 — CRÉATION DE TOUTES LES TABLES (LOCAL ET RENDER)
# ─────────────────────────────────────────────────────────────
...
def creer_toutes_les_tables() -> None:
    ...
    Cette fonction est utilisée à la fois en local et sur Render.
    """
...
# ─────────────────────────────────────────────────────────────
# ÉTAPE 9 — INITIALISATION DE L'APPLICATION (ADAPTÉE POUR RENDER)
# ─────────────────────────────────────────────────────────────
...
def initialiser_application() -> None:
    ...
    - LOCAL : Crée la base de données ET toutes les tables
    - RENDER : Crée uniquement les tables (la base est déjà provisionnée)

    La détection se fait via la variable d'environnement RENDER
    qui est automatiquement définie par Render lors du déploiement.
    """
    logger.info("Initialisation de DMAKeynotesManager...")

    # Étape 9.1 — Détection de l'environnement
    """
    Render définit automatiquement la variable RENDER='true'
    lors du déploiement. En local, cette variable n'existe pas.
    """
    est_sur_render = os.getenv("RENDER", "false").lower() == "true"

    if est_sur_render:
        # ⚠️ SUR RENDER : La base existe déjà, on crée uniquement les tables
        logger.info("=== ENVIRONNEMENT RENDER DÉTECTÉ ===")
        logger.info("La base de données est déjà provisionnée par Render.")
        logger.info("Création des tables uniquement...")

        creer_toutes_les_tables()

        logger.info("✅ Initialisation terminée sur Render.")
    else:
        # 🖥️ EN LOCAL : Création complète (base + tables)
        logger.info("=== ENVIRONNEMENT LOCAL DÉTECTÉ ===")
        logger.info("Création de la base de données et des tables...")

        creer_base_de_donnees()
        creer_toutes_les_tables()

        logger.info("✅ Initialisation terminée en local.")
