...
const Keynotes: React.FC = () => {
    ...
    return projetsPourTableau.map((projet) => {
      ...
      return (
        <React.Fragment key={projet.id}>
          ...
          {/* LIGNE + NOUVELLE CATÉGORIE — au-dessus de la 1ère catégorie */}
          {estProjetOuvert && (
            <tr
              className="add-row"
              style={{ cursor: 'pointer' }}
              onClick={(e) => {
                e.stopPropagation();
                // Sélectionner le projet et activer mode création catégorie
                setIdProjetSelectionne(projet.id);
                reinitialiserFormulaires();
                afficherNotification('Remplissez le formulaire catégorie et cliquez Enregistrer', 'info');
              }}
            >
              <td colSpan={2} style={{ color: '#090ea0', fontSize: '13px', paddingLeft: '8px' }}>
                + Nouvelle catégorie
              </td>
            </tr>
          )}
              ...
              return (
                <React.Fragment key={categorie.id}>
                  ...
                  <tr
                    ...
                    onDoubleClick={(e) => {
                      e.stopPropagation();
                      // Sélectionner ET activer édition directement
                      setIdProjetSelectionne(categorie.id_projet);
                      setCategorieSelectionnee(categorie);
                      setNoteSelectionnee(null);
                      setTypeSelection('categorie');
                  ...
                  {/* LIGNE + NOUVELLE NOTE — au-dessus de la 1ère note */}
                  {estCategorieOuverte && (
                    <tr
                      className="add-row"
                      style={{ cursor: 'pointer' }}
                      onClick={(e) => {
                        e.stopPropagation();
                        selectionnerCategorie(categorie);
                        afficherNotification('Remplissez le formulaire note et cliquez Enregistrer', 'info');
                      }}
                    >
                      <td colSpan={2} style={{ color: '#090ea0', fontSize: '13px', paddingLeft: '32px' }}>
                        + Nouvelle note
                      </td>
                    </tr>
                  )}
  ...
  // ============================================================
  // SECTION 20B — RACCOURCIS CLAVIER Enter / Echap
  // ============================================================
